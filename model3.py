...
class ESRGAN:
    ...
    def build_models(self):
        # Generator
        lr_input = Input(shape=(None, None, 3))  # RGB input
        ...
        # Final conv
        sr_output = layers.Conv2D(3, (3, 3), padding='same', activation='tanh')(trunk)
...
class DataLoader:
    ...
    def _load_and_process(self, path):
        ...
        img = tf.cast(img, tf.float32) / 127.5 - 1  # [-1, 1] range
        
        # Random crop
        img = tf.image.random_crop(img, [self.hr_size, self.hr_size, 3])
